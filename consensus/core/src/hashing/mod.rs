...
pub use header::calculate_header_hash;
pub use sighash::calc_transaction_sighash;
pub use sighash_type::SigHashType;
pub use tx::calc_transaction_hash;

/// Performs a double SHA256 hash on input bytes
pub fn double_sha256(input: &[u8]) -> Hash {
    let first_hash = Sha256::digest(input);
    let second_hash = Sha256::digest(&first_hash);
    // convert the digest output slice into our Hash type
    Hash::try_from_slice(&second_hash).expect("SHA256 output has correct length")
}
...
impl<T: sha2::Digest> HasherExtensions for T {
    #[inline(always)]
    fn write_len(&mut self, len: usize) -> &mut Self {
    ...
    #[inline(always)]
    fn write_blue_work(&mut self, work: BlueWorkType) -> &mut Self {
        // Uint192 exposes `to_bytes()` which returns little-endian bytes in this codebase.
        // Convert to big-endian representation by reversing the little-endian result.
        let mut be_bytes = work.to_bytes();
        be_bytes.reverse();
