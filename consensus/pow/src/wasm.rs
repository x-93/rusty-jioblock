...
use js_sys::BigInt;
use jio_consensus_client::Header;
use jio_consensus_client::HeaderT;
...
/// Represents a Jio header PoW manager
/// @category Mining
...
pub fn calculate_target(difficulty: f32) -> Result<BigInt> {
    ...
    // Build U256 from 4 little-endian u64 limbs
    let mut val = U256::from(buf[0]);
    val |= U256::from(buf[1]) << 64;
    val |= U256::from(buf[2]) << 128;
    val |= U256::from(buf[3]) << 192;
    val.try_into().map_err(Error::custom)
}